...
const bot = new TelegramBot(BOT_TOKEN, { polling: true }); // ✅ POLLING ENABLED FOR FAST RESPONSE
...
// ALL 5 LANGUAGES - EXACT SAME TEXT, ONLY BOLD ADDED
const languages = {
  en: {
    name: "English", flag: "🇺🇸",
...
// ALL PREDICTION IMAGES - EXACT SAME
...
// Player verification - EXACT SAME
...
// Webhook route - EXACT SAME
app.post('/webhook', (req, res) => {
  bot.processUpdate(req.body);
  res.sendStatus(200);
});

// Setup webhook automatically - EXACT SAME
...
bot.onText(/\/start/, async (msg) => {
  ...
  // ✅ FIRST SHOW LANGUAGE SELECTION (FIXED)
  const options = {
    parse_mode: 'Markdown',
    reply_markup: {
      inline_keyboard: [
  ...
  await bot.sendMessage(chatId, languages[lang].selectLanguage, options);
});

// Handle callbacks - EXACT SAME FLOW
...
// Send prediction function - EXACT SAME FORMAT
...
// Handle player ID input - EXACT SAME VERIFICATION FLOW
bot.on('message', async (msg) => {
  if (msg.text && /^\d+$/.test(msg.text)) {
    ...
    const loadingMsg = await bot.sendMessage(msg.chat.id, languages[lang].checking, { parse_mode: 'Markdown' });
...
// Daily motivational messages - EXACT SAME
